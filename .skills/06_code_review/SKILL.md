...
| **P1** | 单元测试覆盖 | `kdit/` 核心代码变更**必须**包含对应测试，否则阻塞 |
...
2. **判定测试要求** — 检查 diff 是否涉及 `kdit/` 核心代码；若涉及，diff 中**必须**包含 `tests/kdit/` 下的测试变更，否则直接标记为 P1 阻塞项
3. **按优先级逐项检查** — P0 架构合规 → P1 逻辑正确性 + 单元测试覆盖 → P2 安全漏洞 → P3 代码质量
4. **查阅关联规范** — 根据改动模块查阅对应的架构和编码规范文档
5. **输出评审结果** — 按优先级列出发现的问题，附带具体文件/行号和修复建议
