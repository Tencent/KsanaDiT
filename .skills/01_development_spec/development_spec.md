...
├── 01_requirements.md         # 第 1 步讨论内容的完整记录
├── 02_feasibility.md          # 第 2 步讨论内容的完整记录
├── 03_design.md               # 第 3 步讨论内容的完整记录
├── 04_test_design.md          # 第 4 步讨论内容的完整记录
├── 05_implementation.md       # 第 5 步实现过程的记录
├── 06_verification.md         # 第 6 步校验结果的记录
└── 07_doc_sync.md             # 第 7 步文档更新的记录
```
...
- **环节 md 文件**：按 `序号_英文环节名.md` 格式命名（如上所示），名称不可重复。每个环节的讨论内容、结论、决策过程都记录在对应的环节文件中，供用户阅读回顾
...
- **讨论过程记录**：每个环节与用户讨论的完整内容、决策过程、备选方案对比等
- **用户阅读材料**：供用户随时回顾某个环节的详细讨论内容
- **结论来源**：环节 md 中的结论会被提炼写入 checklist.md
...
4. 创建各环节 md 文件模板（`01_requirements.md` 到 `07_doc_sync.md`）
5. 后续每步的讨论内容写入对应环节 md 文件，确认结论提炼写入 checklist.md

### 第 1 步：需求澄清

> 📋 **checklist 操作**：
> - 开始前：读取整个 checklist.md，**回顾并打钩第 0 步**的所有条目
> - 过程中：将讨论内容写入 `01_requirements.md`
...
> - 过程中：将实现记录写入 `05_implementation.md`，按 checklist "实现顺序"逐项推进
...
5. 将实现过程的关键决策和变更记录到 `05_implementation.md`
6. **产出**：通过 pre-commit + 全部测试的完整实现代码
...
> - 过程中：逐项审查整个 checklist.md，将校验结果写入 `06_verification.md`
> - 结束时：将校验确认项写入 checklist.md 第 6 步章节（保持 `- [ ]` 未打钩）

1. 读取 checklist.md，逐项检查"设计模块"/"实现顺序"/"测试模块"中的所有条目
2. 确保每一项都已经与用户确认过，没有遗漏
3. 如有未确认项，回到对应步骤与用户补齐确认
4. 将校验结果记录到 `06_verification.md`
