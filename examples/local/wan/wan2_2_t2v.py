...
def run_simple(args):
    pipeline = Pipeline.from_models(
        f"{args.model_dir}/Wan2.2-T2V-A14B", dist_config=DistributedConfig(num_gpus=NUM_GPUS)
    )
...
if __name__ == "__main__":
    """examples:
    - single card run:
        python examples/wan/wan2_2_t2v.py
    - run with multi-gpus has two ways:
        - CUDA_VISIBLE_DEVICES=4,5 python examples/wan/wan2_2_t2v.py
        - CUDA_VISIBLE_DEVICES=4,5 torchrun --nproc_per_node=2 examples/wan/wan2_2_t2v.py
    """
