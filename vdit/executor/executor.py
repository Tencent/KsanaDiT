...
import torch
from ..distributed.utils import get_rank, get_world_size
from ..utils import log, print_recursive
import random
import sys
import math
...
class vDitExecutor(ABC):
    ...
    def __init__(self, **kwargs):
        ...
        shard_fn = None #partial(shard_model, device_id=device_id)
        convert_model_dtype = kwargs.get("convert_model_dtype", None)
    ...
    #              cache_args=None):
        ...
        """
        log.info(f"seed: {seed}, sample_shift: {sample_shift}, sample_guide_scale: {sample_guide_scale}, denoise: {denoise}, "
        ...
        assert sample_solver in ["uni_pc", "dpm++"], f"sample_solver {sample_solver} not supported in list: ['uni_pc', 'dpm++']"
        log.info(f"latents, positive, negtive:")
        print_recursive(latents)
        print_recursive(positive)
        print_recursive(negative)
        high_cache = create_cache(f"{high_model.model_kind}-high", high_model.task_type, high_model.model_size, high_cache_config)
        if low_model is not None:
            low_cache = create_cache(f"{low_model.model_kind}-low", low_model.task_type, low_model.model_size, low_cache_config)
            low_sample_guide_scale = low_sample_guide_scale if low_sample_guide_scale is not None else low_model.default_config.sample_guide_scale[0]
        ...
        seq_len = math.ceil((target_shape[2] * target_shape[3]) /
                            ...
                            target_shape[1] / self.sp_size) * self.sp_size # 7200
        ...
        with (
            ...
            sample_scheduler, _, timesteps = get_sample_scheduler(
                ...
                sampling_steps=sampling_steps, sample_solver=sample_solver, device=self.run_device, shift=sample_shift)
            
            arg_c = {'phase': 'cond', 'context': [positive], 'seq_len': seq_len} #, seq_len:720, 与frame_num相关
            ...
            for iter_id, t in enumerate(tqdm(timesteps)):
                ...
                timestep_id = t.item()
                run_model = self.get_run_model(high_model=high_model, low_model=low_model, timestep_id=timestep_id, boundary=boundary)
                run_model = self.cast_to(run_model, run_dtype, self.run_device)
                run_cache = self.get_run_cache(high_cache=high_cache, low_cache=low_cache, timestep_id=timestep_id, boundary=boundary)

                noise_pred_cond = run_model.forward(
                    latent_model_input, t=timestep, cache=run_cache, **arg_c)[0]
                noise_pred_uncond = run_model.forward(
                    latent_model_input, t=timestep, cache=run_cache, **arg_null)[0]
                ...
                temp_x0 = sample_scheduler.step(
                    ...
                    return_dict=False,
                    generator=seed_g)[0] # [1, 16, 2, 90, 160]
                latents = [temp_x0.squeeze(0)]
            # if cache is not None:
            #     for cache_name, cache in cond_cache.items():
