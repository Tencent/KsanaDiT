...
DCACHE_COEFFS_MAPS={
    "wan2.2-high" : {
        "t2v" : {
            "A14B" : [1.79787941e-06, -6.73669299e-03,  9.45476817e+00, -5.89111662e+03, 1.37508591e+06]
        }
    },
    "wan2.2-low" : {
        "t2v" : {
            "A14B" : [-4.67923490e-09,  9.48489344e-06, -6.97084940e-03,  2.12416998e+00, -1.65571475e+02]
        }
    }
}
def get_coeffs(model_kind:str, model_type:str, model_size:str,):
    try:
...
class DCache(vDitCache):
    ...
    def __str__(self):
        return f"DitCache {self.name} timestep_range: [{self.timestep_start}, {self.timestep_end}] " \
    ...
    def can_use_cache(self, phase: str, current_x_input, current_timestep: int) -> bool:
        ...
        if self.total_cnt[phase] <= self.config.skip_first_n_iter:
            log.info(f"[NOT use cache] phase {phase} timestep {current_timestep}, by skip first {self.config.skip_first_n_iter} iter, current iter {self.total_cnt[phase]}")
            return False
        if self.prev_diff[phase] is None:
            log.info(f"[NOT use cache] phase {phase} timestep {current_timestep}, by prev_diff is None")
            return False
        ...
        if cur_degree > self.config.fast_degree:
            log.info(f"[NOT use cache] phase {phase} timestep {current_timestep}, by degree {cur_degree:.1f} > fast degree {self.config.fast_degree}")
            return False
        else:
            return True
    ...
    def try_get_prev_cache(self, phase: str, current_x_input, current_timestep: int):
        ...
        if self.config.slow_degree < self.cur_degree <= self.config.fast_degree and self.cnt_continuous_cached[phase] >= self.config.fast_force_calc_every_n_steps:
            log.info(f"[NOT use cache] phase {phase} timestep {current_timestep}, by every {self.config.fast_force_calc_every_n_steps} steps")
            self.cnt_continuous_cached[phase] = 0
            return None
        if self.cur_degree <= self.config.slow_degree and self.cnt_continuous_cached[phase] >= self.config.slow_force_calc_every_n_steps:
            log.info(f"[NOT use cache] phase {phase} timestep {current_timestep}, by every {self.config.slow_force_calc_every_n_steps} steps")
            self.cnt_continuous_cached[phase] = 0
            return None
        log.info(f"[USE cache] phase {phase} timestep {current_timestep}, cur_degree {self.cur_degree:.1f}")
    ...
    def show_cache_rate(self):
        ...
        print(f"DCache {self.name} fast degree {self.config.fast_degree}, slow degree {self.config.slow_degree}, "\
