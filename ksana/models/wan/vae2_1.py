...
def _video_vae(pretrained_path=None, z_dim=None, device="cpu", **kwargs):
    ...
    unified_sd = load_state_dict_and_merge_lora(pretrained_path, device=device)
    load_state_dict(model, unified_sd, assign=True, strict=True)

    return model


class Wan2_1_VAE:  # pylint: disable=invalid-name
    def __init__(
        self,
        z_dim=16,
        ...
        if latents_mean is None or latents_std is None:
            raise ValueError("latents_mean and latents_std must be provided (from vae_2_1.yaml)")
        self.mean = torch.tensor(list(latents_mean), dtype=dtype, device=device)
        self.std = torch.tensor(list(latents_std), dtype=dtype, device=device)
