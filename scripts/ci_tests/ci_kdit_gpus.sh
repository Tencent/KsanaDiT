...
# Run kdit multi-GPU tests.
#
# CI usage (unchanged):
#   bash ci_kdit_gpus.sh <commit_id> [gpu_cards] [test_port]
#
# Local usage:
#   bash ci_kdit_gpus.sh                # auto-detect project root, GPUs=0,1
#   bash ci_kdit_gpus.sh "" 2,3 29501   # auto-detect project root, GPUs=2,3, port=29501
#   KSANA_CI_MODELS_ROOT=/my/models bash ci_kdit_gpus.sh
...
export RAY_TMPDIR=/tmp/ray_kdit
...
cd "${WORK_DIR}"
pytest -s -v tests/kdit/pipelines/

for file in "tests/kdit/nodes/*.py"; do
    [ -e "$file" ] || continue
    echo "running $file"
